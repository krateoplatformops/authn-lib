...
type SecretsKeeper interface {
	...
	Get(kid string) (key any, algorithm string, err error)
}

// IssueToken issue jwt access token from the given dest.
func IssueToken(k SecretsKeeper, dest ...any) (string, error) {
	kid := k.KID()
...
func ParseToken(k SecretsKeeper, token string, dest ...any) error {
