...
import (
	...
	"github.com/krateoplatformops/authn-lib/auth/claims"
	"github.com/krateoplatformops/authn-lib/auth/strategies/oauth2"
	"github.com/krateoplatformops/authn-lib/auth/strategies/oauth2/introspection"
...
func Example() {
	...
	strategy := introspection.New(srv.URL, gcache.New(0).Simple().Build(), opt)
...
func ExampleSetVerifyOptions() {
	...
	strategy := introspection.New(srv.URL, gcache.New(0).Simple().Build(), client, vopts)
	r, _ := http.NewRequest("GEt", "/protected/resource", nil)
	r.Header.Set("Authorization", "Bearer <oauth2-token>")
	info, err := strategy.Authenticate(r.Context(), r)
