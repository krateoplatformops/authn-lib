...
import (
	...
	"github.com/krateoplatformops/authn-lib/auth/strategies/oauth2/jwt"
...
func Example() {
	...
	strategy := jwt.New(srv.URL, gcache.New(10).Simple().Build(), opt)
...
func ExampleSetVerifyOptions() {
	...
	strategy := jwt.New(srv.URL, gcache.New(10).Simple().Build(), client, vopts)
	r, _ := http.NewRequest("GEt", "/protected/resource", nil)
	r.Header.Set("Authorization", "Bearer "+accessToken)
	info, err := strategy.Authenticate(r.Context(), r)
