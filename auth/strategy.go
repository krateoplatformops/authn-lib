...
type OptionFunc func(v any)

// Apply the configuration to the provided strategy.
func (fn OptionFunc) Apply(v any) {
	fn(v)
}
...
func Append(s Strategy, key any, info Info) error {
	u, ok := s.(interface {
		Append(any, Info) error
	})
...
func Revoke(s Strategy, key any) error {
	u, ok := s.(interface {
		Revoke(key any) error
	})
